function [outputFile] = processDataFileRepeatSwitch(logFileDict,theXLFile,sheetName)
% Function create a csv file with new columns populated based on the
% contents of a log file
%
% Notes: 1. this is correct assuming SYNAP4 experiment only
...
elseif strfind(lower(sheetName),'well')
    ...
    error('processDataFileSynap4: neither well nor field found in sheet name');
end
...
for ii = 1:numel(files)
    ...
    if ~isempty(regexp(aFile,'Co-localization.*xls','once','ignorecase'))
        if coLocfileFound
            warning('processDataFileSynap4: multiple coLocalization files found in %s',dirPath); %#ok<WNTAG>
        end
        coLocfileFound = true;
    ...
    if ~isempty(regexp(aFile,'NeuN cFos.*xls','once','ignorecase'))
        if neuNfileFound
            warning('processDataFileSynap4: multiple NeuN files found in %s',dirPath); %#ok<WNTAG>
        end
        neuNfileFound = true;
...
for row = startRow:rows
    ...
    % for synap4, assume all are 'inhib'
    switch colNumber
        case {2 11}
            marker = 'GADGeph';
        case{3 10}
    ...
    switch rowLetter
        ...
        otherwise
            warning('processdataFile: something weird with treatment'); %#ok<WNTAG>
            treatment = [];
    end
    ...
    % Switch Puncta 1 data with Puncta 2 data
    if strcmp(marker,'GADGeph')
        % Synap4 sheet data
        tmp = theRow{cOrg1Count};
        theRow{cOrg1Count} = theRow{cOrg2Count};
        theRow{cOrg2Count} = tmp;
        tmp = theRow{cOrgInt1};
        theRow{cOrgInt1} = theRow{cOrgInt2};
        theRow{cOrgInt2} = tmp;
        % newdata is written to an output sheet
        % correct changes
        tmp = newdata{row+offset,5};
        newdata{row+offset,5} = newdata{row+offset,7};
        newdata{row+offset,7} = tmp;
        tmp = newdata{row+offset,6};
        newdata{row+offset,6} = newdata{row+offset,8};
        newdata{row+offset,8} = tmp;
    end
    
    if strcmp(marker,'GADgamma')
        tmp = theRow{cOrg1Count};
        theRow{cOrg1Count} = theRow{cOrg2Count};
        theRow{cOrg2Count} = tmp;
        tmp = theRow{cOrgInt1};
    ...
    if colocalize
        ...
        % Switch Puncta 1 data with Puncta 2 data
        if strcmp(marker,'GADGeph')
        tmp = coLocData{coLocRow,coLocCol1};
        coLocData{coLocRow,coLocCol1} = coLocData{coLocRow,coLocCol2};
        coLocData{coLocRow,coLocCol2} = tmp;
        end
        if strcmp(marker,'GADgamma')
        tmp = coLocData{coLocRow,coLocCol1};
        coLocData{coLocRow,coLocCol1} = coLocData{coLocRow,coLocCol2};
